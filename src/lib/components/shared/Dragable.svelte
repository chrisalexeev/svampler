<script lang="ts">
    ...
    function onHandleChange(newHandles: HTMLElement[], newAntiHandles: HTMLElement[]) {
        handles_ = newHandles.filter((handle) => !antiHandles_.includes(handle));
        antiHandles_ = newAntiHandles;
    }
