...
export class Voice {
	...
	constructor(context: any, type = "sawtooth", gain = 0.1) {
		this.context = context;
		this.type = type;
		this.value = -1;
		this.gain = gain;
	...
	on(MidiEvent: { frequency: any; velocity: any; value?: any; }) {
...
export class Noise extends Voice {
	_length: number;
	constructor(context: OfflineAudioContext, gain: number | undefined) {
		super(context, undefined, gain);
		this._length = 2;
	}
	...
	on(MidiEvent: { frequency?: number; velocity: any; value?: any; }) {
		this.value = MidiEvent.value;
		this.ampEnvelope.on(MidiEvent.velocity || MidiEvent);
	}
...
export class Filter extends Effect {
	declare effect: BiquadFilterNode;
	constructor(context: AudioContext | OfflineAudioContext, type: BiquadFilterType = "lowpass", cutoff = 1000, resonance = 0.9) {
		super(context);
		this.name = "filter";
